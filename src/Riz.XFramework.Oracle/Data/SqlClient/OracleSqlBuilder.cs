...
namespace Riz.XFramework.Data.SqlClient
{
    ...
    internal class OracleSqlBuilder : SqlBuilder
    {
        ...
        public OracleSqlBuilder(ITranslateContext context)
            : base(context)
        {
            var provider = context.DbContext.Provider;
            _escCharLeft = provider.QuotePrefix;
            _escCharRight = provider.QuoteSuffix;
            _caseSensitive = context != null && context.DbContext != null ? ((OracleDbContext)context.DbContext).CaseSensitive : false;
        }
