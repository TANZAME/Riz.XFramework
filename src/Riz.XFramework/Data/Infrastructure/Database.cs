...
namespace Riz.XFramework.Data
{
    ...
    public partial class Database : IDatabase, IDisposable
    {
        ...
        protected IEnumerable<IDbDataParameter> GetParameters(string sql, params object[] args)
        {
            ...
            string pattern = string.Format("[\\s>=<]+{0}(?<ParameterName>[0-9a-zA-Z_]+)", _parameterPrefix);
            ...
            else
            {
                ...
                {
                    var m = matches[i];
                    var parameter = _provider.CreateParameter(string.Format("{0}{1}", _parameterPrefix, m.Groups["ParameterName"].Value.Trim()), args[i]);
