    ...
    internal class ColumnExpressionVisitor : DbExpressionVisitor
    {
        ...
        private void VisitMemberAssignmentImpl(Type newType, MemberAssignment m)
        {
            ...
            else
            {
                ...
                if (m.Expression.NodeType == ExpressionType.MemberAccess && m.Expression.CanBeRecurse())
                {
                ...
                if (!_navDescriptors.Contains(keyId))
                {
                    ...
                    nav.FieldCount = GetFieldCount(m.Expression) + (m.Expression.NodeType == ExpressionType.MemberAccess && m.Expression.CanBeRecurse() ? 1 : 0);
                ...
                // 2.表达式里显式指定导航属性，例：b
                if (m.Expression.NodeType == ExpressionType.MemberAccess) this.VisitNavigation(m.Expression as MemberExpression, m.Expression.CanBeRecurse());
