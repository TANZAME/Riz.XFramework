...
{
    public class MySqlTestN : TestBaseN<MySqlModelN.MySqlDemo>
    {
        ...
        protected override void Parameterized()
        {
            ...
            // 构造函数
            var query =
                 from a in context.GetTable<MySqlModelN.MySqlDemo>()
                 where a.RizDemoId <= 10
                 select new MySqlModelN.MySqlDemo(a);
            var r1 = query.ToList();
            query =
               from a in context.GetTable<MySqlModelN.MySqlDemo>()
               where a.RizDemoId <= 10
               select new MySqlModelN.MySqlDemo(a.RizDemoId, a.RizDemoName);
            r1 = query.ToList();
            //SQL=> 
            //SELECT 
            //t0.[DemoId] AS [DemoId],
        ...
        protected override void API()
        {
            ...
            var demos = new List<MySqlModelN.MySqlDemo>();
            for (int i = 0; i < 5; i++)
            {
                MySqlModelN.MySqlDemo d = new MySqlModelN.MySqlDemo
                {
            ...
            context.Insert<MySqlModelN.MySqlDemo>(demos);
            context.SubmitChanges();
            var myList = context
                .GetTable<MySqlModelN.MySqlDemo>()
                .OrderByDescending(x => x.RizDemoId)
                .Take(5).ToList();
            Debug.Assert(myList[0].DemVarBinary_s == LongText.LONGTEXT);

            // byte[]
            var demo = new MySqlModelN.MySqlDemo
            {
                RizDemoCode = "D0000001",
                RizDemoName = "N0000001",
                RizDemoBoolean = true,
            ...
            demo = context.GetTable<MySqlModelN.MySqlDemo>().FirstOrDefault(x => x.RizDemoId == demo.RizDemoId);
