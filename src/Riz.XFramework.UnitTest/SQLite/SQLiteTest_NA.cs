...
namespace Riz.XFramework.UnitTest.SQLite
{
    public class SQLiteTest_NA : TestBase_NA<SQLiteModel_NA.Demo>
    {
        ...
        protected override void Parameterized()
        {
            ...
            // 构造函数
            var query =
                 from a in context.GetTable<SQLiteModel_NA.Demo>()
                 where a.RizDemoId <= 10
                 select new SQLiteModel_NA.Demo(a);
            var r1 = query.ToList();
            //SQL=> 
            //SELECT 
            //t0.[DemoId] AS [DemoId],
            ...
            query =
               from a in context.GetTable<SQLiteModel_NA.Demo>()
               where a.RizDemoId <= 10
               select new SQLiteModel_NA.Demo(a.RizDemoId, a.RizDemoName);
            r1 = query.ToList();
            //SQL=>
            //SELECT 
            //t0.[DemoId] AS [DemoId],
        ...
        protected override void API()
        {
            ...
            var demos = new List<SQLiteModel_NA.Demo>();
            for (int i = 0; i < 5; i++)
            {
                SQLiteModel_NA.Demo d = new SQLiteModel_NA.Demo
                {
            ...
            context.Insert<SQLiteModel_NA.Demo>(demos);
            context.SubmitChanges();
            var myList = context
                .GetTable<SQLiteModel_NA.Demo>()
                .OrderByDescending(x => x.RizDemoId)
                .Take(5).ToList();

            // byte[]
            var demo = new SQLiteModel_NA.Demo
            {
                RizDemoCode = "D0000001",
                RizDemoName = "N0000001",
                RizDemoBoolean = true,
            ...
            demo = context.GetTable<SQLiteModel_NA.Demo>().FirstOrDefault(x => x.RizDemoId == demo.RizDemoId);
        }
    }
}
